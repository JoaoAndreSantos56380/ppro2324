...
main =  do
        x <- getArgs
        --if (head x == "-t") then undefined else do
...
--testes = do
        ...
        --verboseCheck prop_directions

prop_directions :: Movimentos -> Bool
prop_directions x = (length (movimentosToString x)) < 31


newtype Movimentos = Movimentos String deriving Show

movimentosToString :: Movimentos -> String
movimentosToString (Movimentos x) = x
...
instance Arbitrary EstadoJogo where
        arbitrary = do
                    m <- choose (3,8)
                    inicializa (preencheVazia (m*m) 0)
                    return ()
...
preencheLab xs ys = do 
                ...
                insertChar p (ys !! 4) 0 'a'

listaCaracteres :: Int -> Int -> Int -> Int -> Int -> Int -> String
listaCaracteres portal portas nChaves paredes count tamanho = do
