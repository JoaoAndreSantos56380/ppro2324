...
type Estrategia = Int -> Baralho -> Baralho -> (Int, Bool)

sempreStand :: Estrategia
sempreStand _ _ _ = (5, False)

sempreHit :: Estrategia
sempreHit _ _ _ = (5, True)

blackjacktstrat :: Estrategia
blackjacktstrat playerCredits _ _ = (5, playerCredits > 1)
...
simulaRonda e state@(EstadoJogo {playerCredits, currentBet, deck, playerHand, dealerHand})
  ...
  | snd (e playerCredits playerHand dealerHand) = state {playerHand = nextCard : playerHand, deck = nextDeck}
  ...
  if convenientHandValue dealerHand >= 17
    then
      if convenientHandValue playerHand > convenientHandValue dealerHand
        then state {playerCredits = playerCredits + currentBet * 2, currentBet = 0, playerHand = [], dealerHand = []}
        else
          if convenientHandValue playerHand == convenientHandValue dealerHand
            then state {playerCredits = playerCredits + currentBet, currentBet = 0, playerHand = [], dealerHand = []}
            else
              if convenientHandValue dealerHand > 21
                then state {playerCredits = playerCredits + currentBet * 2, currentBet = 0, playerHand = [], dealerHand = []}
                else state {currentBet = 0, playerHand = [], dealerHand = []}
